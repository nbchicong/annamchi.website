...
function showCart($view) {
 ...
 $rowstr = mysql_fetch_array($resultstr);
	...
	if ($cart) {
		...
		$output[] = '<p>Đơn hàng này không có gì hêt.</p>';
	}		
	return join('',$output); 
}
?>
...
<tr >
 ...
 <div style="width:96%; padding-top:20px" align="left"><strong>Thông tin người gửi đơn hàng:</strong></div>
<?

$sqlstr=mysql_query("SELECT * FROM ".shoppingCart." WHERE id=".intval($_GET['view'])."" );
mysql_query("UPDATE ".shoppingCart." SET status='true' WHERE id=".intval($_GET['view'])."" );
if(mysql_num_rows($sqlstr)>0) {

$row=mysql_fetch_array($sqlstr);
?>
<div style="float:left; padding-top:20px">
<table border="1" bordercolor="#bbbbbb" bgcolor="#EEEEEE" align="left" cellpadding="1" cellspacing="1" width="750">
<tr>
...
<tr>
<td><div align="right">ĐT bàn</div></td>
<td><?=$row['telephone']?>&nbsp;</td>
</tr>

<tr>
<td><div align="right">DTDD</div></td>
<td><?=$row['mobiphone']?>&nbsp;</td>
</tr>
<tr>
...
<tr id="tieude">
      <td width="5%" height="27" ><div align="center">#ID</div></td>
        <td width="20%" >Người gửi</td>
        <td width="17%" ><div align="center">Ngày gửi </div></td>
        <td width="20%" ><div align="center">Điện thoại </div></td>
        <td width="10%" ><div align="center">Email </div></td>
        <td width="10%" ><div align="center">Địa chỉ </div></td>
...
?>	  
	   <tr >
        ...
        <td  ><a href="index.php?site=<?=$_GET['site']?>&view=<?=$row['id']?>"><?=$row['telephone']?></a></td>
        <td  ><a href="index.php?site=<?=$_GET['site']?>&view=<?=$row['id']?>"><?=$row['email']?></a></td>
        <td  ><a href="index.php?site=<?=$_GET['site']?>&view=<?=$row['id']?>"><?=$row['address']?></a></td>
        <td width="100" style="cursor:pointer" ><div align="center"><? echo $row['status']=='true'?'Đã xem':'Chưa xem';?></div></td>
...
?> 
 <tr>
        ...
        <td colspan="5"  align="right"><? view_page($link)?></td>
      </tr>		
<tr>
